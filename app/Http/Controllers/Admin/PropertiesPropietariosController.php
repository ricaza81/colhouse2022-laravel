 ...
 /**
    ...
    public function index($id)
    {
        ...
        if (request('show_deleted') == 1) {
            ...
            $seguros = PropertiesPropietarios::where('deleted_at',NULL)->where('id_property','=',$id)->get();
...
/*    public function indextotal()
    {
        ...
        if (request('show_deleted') == 1) {
            ...
            $property=Property::where('deleted_at',NULL)->get();
        ...
        return view('admin.properties_propietarios.indextotal', compact('property'));
    }
    */

    public function indextotalpropietarios()
    {
        if (! Gate::allows('properties_seguros_access')) {
            return abort(401);
        }
        ...
        if (request('show_deleted') == 1) {
            ...
            $propietarios=PropertiesPropietarios::where('deleted_at',NULL)->get();
        } else {
         $propietarios=PropertiesPropietarios::where('deleted_at',NULL)->get();
         //$propietarios_unicos_detalle=DB::table('properties_propietarios')->distinct('cedula')->get();
         $propietarios_unicos_detalle=PropertiesPropietarios::where('deleted_at',NULL)->get();
         $propietarios_unicos_total=DB::table('properties_propietarios')->distinct()->count('cedula');
         $cuenta_propietarios_total=DB::table('properties_propietarios')->distinct()->count('id');
         $propietarios_multi=$cuenta_propietarios_total-$propietarios_unicos_total;

         if ($propietarios_multi > 0)
          {
               foreach ($propietarios as $propietario) {
                    $propietarios = $propietarios;
                    $propietarios_unicos=PropertiesPropietarios::where('id',$propietario->id)->where('deleted_at',NULL)->get();
                    $properties=Property::where('id',$propietario->id_property)->get();
                } 
               // return ($properties);
                /*else {
        ...
        return view('admin.properties_propietarios.indextotalpropietarios', compact('propietarios','propietarios_multi','propietarios_unicos','propietarios_unicos_detalle','propietarios_unicos_total'
        ));
    }
