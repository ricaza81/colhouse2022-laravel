...
class TenantsController extends Controller
{
    ...
    /*Version 0*/
     public function informe_inquilinos_consulta_propietario($id)
    {
        $fecha_inicio_informe = 0;
        $fecha_fin_informe = 0;
        //$propiedad=Property::where('cedula_propietario',19055077)->get();
        $propietario=PropertiesPropietarios::where('cedula',$id)->first();
        $cc_propietario=$propietario->cedula;
        //dd($propietario);
        //$id_propiedad=$propiedad->id;
        //$propietario=$propiedad->propietarios2($id_propiedad);
        //$id_propietario=$id;
       $tenants=User::where('cedula_propietario','=',$cc_propietario)->where('role_id',3)->orderBy('property_id','DESC')->get();
        //$ingresos=12000;
        $ingresos=DB::table('properties_pagos')->where('cedula_propietario','=',$cc_propietario)->where('deleted_at','=',NULL)->sum('valor');
        $administracion=$ingresos*$propietario->porc_comision/100;
        $deducciones=PropietariosDeducciones::where('cedula_propietario','=',$cc_propietario)->sum('valor');
        $deducciones_detalles=PropietariosDeducciones::where('cedula_propietario','=',$cc_propietario)->get();
        $valor_pagar=$ingresos-$administracion-$deducciones;
        $id_propietario=$id;

        return view('admin.tenants.informe_inquilinos_consulta_propietario', compact('propietario','tenants','ingresos','administracion','deducciones','valor_pagar','id_propietario','deducciones_detalles','fecha_inicio_informe','fecha_fin_informe','cc_propietario'))
        ->with('msj','');
    }
      ...
      public function informe_inquilinos_consulta($id)
    {
        ...
        $cc_propietario=$propiedad->propietarios2($id_propiedad)->cedula;
        ...
        return view('admin.tenants.informe_inquilinos_consulta', compact('tenants','id_propiedad','ingresos','administracion','deducciones','valor_pagar','id_propietario','deducciones_detalles','fecha_inicio_informe','fecha_fin_informe','propiedad','cc_propietario'))
        ->with('msj','');
    }
    
   
    public function informe_inquilinos_consulta_propietario_fechas(Request $request)
    {
        $input = $request->all();
        $cc_propietario= $input['cc_propietario'];
        //$cc_propietario=$propiedad->propietarios2($id_propiedad)->cedula;
        $propietario=PropertiesPropietarios::where('cedula',$cc_propietario)->first();
        $fecha_inicio_informe = $input['fecha_inicio_informe'];
        $fecha_fin_informe = $input['fecha_fin_informe'];
        //dd($fecha_fin_informe);
        $tenants=User::where('cedula_propietario','=',$cc_propietario)->where('role_id',3)->orderBy('property_id','DESC')->get();
        //$ingresos=12000;
        $ingresos=DB::table('properties_pagos')->where('cedula_propietario','=',$cc_propietario)->where('deleted_at','=',NULL)->where('fecha_pago','>=',$fecha_inicio_informe)->where('fecha_pago','<=',$fecha_fin_informe)->sum('valor');
        $administracion=$ingresos*$propietario->porc_comision/100;
        $deducciones=PropietariosDeducciones::where('cedula_propietario','=',$cc_propietario)->where('fecha_deduccion','>=',$fecha_inicio_informe)->where('fecha_deduccion','<=',$fecha_fin_informe)->sum('valor');
        $deducciones_detalles=PropietariosDeducciones::where('cedula_propietario','=',$cc_propietario)->where('fecha_deduccion','>=',$fecha_inicio_informe)->where('fecha_deduccion','<=',$fecha_fin_informe)->get();
        $valor_pagar=$ingresos-$administracion-$deducciones;
        $id_propietario=$cc_propietario;

        return view('admin.tenants.informe_inquilinos_consulta_propietario', compact('propietario','tenants','ingresos','administracion','deducciones','valor_pagar','id_propietario','deducciones_detalles','fecha_inicio_informe','fecha_fin_informe','propietario','cc_propietario'))
        ->with('msj','');
        /*    $id_propietario=$propietario->id;
        return view('admin.tenants.informe_inquilinos_consulta', compact('tenants','id_propietario','fecha_inicio_informe','fecha_fin_informe','propietarios','propiedades','propietario'))
        ->with('msj','');*/
    }
      ...
      public function informe_inquilinos_consulta_fechas(Request $request)
    {
        ...
        $valor_pagar=$ingresos-$administracion-$deducciones;
        return view('admin.tenants.informe_inquilinos_consulta',compact('tenants','id_propiedad','ingresos','administracion','deducciones','valor_pagar','id_propietario','deducciones_detalles','fecha_inicio_informe','fecha_fin_informe','propietario','propiedad','cc_propietario'))
    ...
    public function imprimir_informe_propietario(Request $request)
      {
        $input = $request->all();
        $cc_propietario= $input['cc_propietario'];
        //$cc_propietario=$propiedad->propietarios2($id_propiedad)->cedula;
      ...
      $pdf= \PDF::loadView('admin.tenants.informe_propietario_consulta_imprimir',compact('propietario','tenants','ingresos','administracion','deducciones','valor_pagar','id_propietario','deducciones_detalles','fecha_inicio_informe','fecha_fin_informe','cc_propietario'));
      return $pdf->download('InformeColhouse-CC'.$propietario->cedula.'-'.$fecha_inicio_informe.'-'.$fecha_fin_informe.'-'.$propietario->nombre.'.pdf');
    }
   ...
   /*return new Excel ('admin.tenants.informe_inquilinos_consulta_imprimir', function($excel) use ($data) {
     ...
     public function enviar_informe(Request $request)
    {
      ...
      //$pdf= \PDF::loadView('admin.tenants.informe_inquilinos_consulta_imprimir',compact('tenants','id_propiedad','ingresos','administracion','deducciones','valor_pagar','id_propietario','deducciones_detalles','fecha_inicio_informe','fecha_fin_informe','propietario'));
        ...
        $id_propiedad = $input['id_propiedad'];
        $id_propietario = $input['id_propietario'];
        $cc_propietario= $input['id_propietario'];
        $propiedad=Property::findOrFail($id_propiedad);
        $id_propiedad=$propiedad->id;
        $tenants=User::where('property_id','=',$id_propiedad)->get();
        $ingresos=$propiedad->facturas_pagadas_propiedad_fechas($propiedad->id, $fecha_inicio_informe,$fecha_fin_informe);
    ...
    public function enviar_informe_propietario(Request $request)
    {
        //$data["email"] = "ricaza81@gmail.com";
        //$data["title"] = "Informe";
        //$data["body"] = "This is the email body.";
      ...
      //$pdf= \PDF::loadView('admin.tenants.informe_inquilinos_consulta_imprimir',compact('tenants','id_propiedad','ingresos','administracion','deducciones','valor_pagar','id_propietario','deducciones_detalles','fecha_inicio_informe','fecha_fin_informe','propietario'));
       ...
       $pdf = \PDF::loadView('admin.tenants.informe_propietario_consulta_enviar', $data);

        Mail::send('admin.tenants.informe_propietario_consulta_enviar',$data,  function ($message) use ($data,$email_propietario,$propietario,$pdf) {
            $message->to
    ($email_propietario,'ricaza81@gmail.com')
    //->cc('laurazambranoduran.abogada@gmail.com')
            //$message->to('ricaza81@gmail.com')
                ->subject('Informe Propietario'.'-'.$propietario->nombre)
                ->attachData($pdf->output(), "InformeColHouse.pdf");
        });
        ...
        $deducciones_detalles=PropietariosDeducciones::where('cedula_propietario','=',$cc_propietario)->where('fecha_deduccion','>=',$fecha_inicio_informe)->where('fecha_deduccion','<=',$fecha_fin_informe)->get();
        $valor_pagar=$ingresos-$administracion-$deducciones;
        return view('admin.tenants.informe_inquilinos_consulta_propietario',compact('tenants','ingresos','administracion','deducciones','valor_pagar','id_propietario','deducciones_detalles','fecha_inicio_informe','fecha_fin_informe','propietario','cc_propietario'))
        ->with('msj','Reporte enviado correctamente al propietario '.$propietario->nombre.' con email '.$email_propietario);
    }

      public function informe_inmobiliaria()
    {
        $fecha_inicio_informe = 0;
        $fecha_fin_informe = 0;
        $usuarioadmin1=\Auth::user();
        $usuarioadmin=User::findOrFail($usuarioadmin1->id);
        //$propiedad=Property::all();
        $propiedad=Property::where('deleted_at',NULL)->get();
        //$propiedades=Property::where('deleted_at',NULL)->get();
        $ingresos=PropertiesPagos::where('fecha_pago','>=',$fecha_inicio_informe)->where('fecha_pago','<=',$fecha_fin_informe)->where('deleted_at',NULL)->sum('valor');

          //<!--IngresosUsuarioAdmin-->
        $idpropiedad_usuarioactual=\Auth::user()->property_id;
        $ingresos_inmobiliaria = PropertiesPagos::where('id_property',$idpropiedad_usuarioactual)->where('fecha_pago','>=',$fecha_inicio_informe)->where('fecha_pago','<=',$fecha_fin_informe)->where('deleted_at',NULL)->sum('valor');
        //<!--IngresosUsuarioAdmin-->

        //$tenants=User::all();
        $administracion=$ingresos*10/100;
        $deducciones=PropietariosDeducciones::where('deleted_at','=',NULL)->sum('valor');
        $deducciones_detalles=PropietariosDeducciones::where('deleted_at','=',NULL)->get();
        $valor_pagar=$administracion-$deducciones;

       // return view('admin.tenants.informe_inmobiliaria', compact('fecha_inicio_informe','fecha_fin_informe','ingresos','tenants','administracion','deducciones','valor_pagar','deducciones_detalles'))

        return view('admin.tenants.informe_inmobiliaria', compact('fecha_inicio_informe','fecha_fin_informe','usuarioadmin','propiedad','ingresos','ingresos_inmobiliaria','administracion','deducciones','valor_pagar','deducciones_detalles'))
        ->with('msj','');
    }

      public function informe_inmobiliaria_consulta_fechas(Request $request)
    {
        $input = $request->all();
        $fecha_inicio_informe = $input['fecha_inicio_informe'];
        $fecha_fin_informe = $input['fecha_fin_informe'];
        ...
        $administracion=$ingresos*10/100;
        $deducciones=PropietariosDeducciones::where('fecha_deduccion','>=',$fecha_inicio_informe)->where('fecha_deduccion','<=',$fecha_fin_informe)->where('deleted_at','=',NULL)->sum('valor');
        $deducciones_detalles=PropietariosDeducciones::where('fecha_deduccion','>=',$fecha_inicio_informe)->where('fecha_deduccion','<=',$fecha_fin_informe)->where('deleted_at','=',NULL)->get();
        $valor_pagar=$administracion-$deducciones;
