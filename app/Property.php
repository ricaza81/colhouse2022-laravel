...
class Property extends Model
{
    ...
    protected $fillable = ['name', 'address', 'photo', 'user_id','id_tipo','id_seguro','id_pais','id_ciudad','cedula_propietario'];
    ...
    public function inquilinos()
     ...
     //return $this->belongsTo('App\PropertiesPropietarios', 'id_property', 'id_property');
         ...
         public function propietarios2($id)
      {
        ...
        $resul=DB::table('properties_propietarios')->where('id_property','=',$id)->first();
        return ($resul);
  ...
  
{
     ...
     public function facturas_pagadas_propiedad_fechas($id,$fecha_inicio_informe,$fecha_fin_informe)
    {
      ...
      public function facturas_pagadas_inmobiliaria($fecha_inicio_informe,$fecha_fin_informe)
    {
        //return $this->hasMany(PropertiesFacturas::class,'$id');
        $resul=DB::table('properties_pagos')->where('deleted_at','=',NULL)->where('fecha_pago','>=',$fecha_inicio_informe)->where('fecha_pago','<=',$fecha_fin_informe)->sum('valor');
        return ($resul);
    }
  ...
  
{
     ...
     return ($valor_deducciones);
        ...
        public function inquilinos_propiedad($id)
      {
      $propiedad=Property::findOrFail($id);
      $result=DB::table('users')->where('property_id','=',$propiedad->id)->where('role_id','>',1)->get();
      return ($result);
      }
