            ...
            <div class="row">
                  ...
                  <div class="col-md-4">
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tr>
                            <th>Fechas Contrato</th>
                            <td field-key='porpagar'>
                            Inicio: {{date('d/F/Y', strtotime($tenant->fecha_inicio_contrato))}}<br/>
                            Fin: {{date('d/F/Y', strtotime($tenant->fecha_fin_contrato))}}
                            </td>
                        </tr>
                        <tr>
                            <!--Prorrogas-->
                            <th>Prórroga: <a href="#" data-toggle="tooltip" title="Si el inquilino no cancelo el contrato, el sistema crea automáticamente la prórroga" class="label label-danger" style="padding:6px;font-weight:700;border-radius:100%;background:#000;padding-left:8px;padding-right:8px;">?</a>
                            <td>
                            @if ($tenant->prorroga_contrato($tenant->id) != null)
                            Desde:<br/>{{date('d/F/Y', strtotime($tenant->prorroga_contrato($tenant->id)->fecha_inicio_prorroga))}}<br/>
                            Hasta:<br/>{{date('d/F/Y', strtotime($tenant->prorroga_contrato($tenant->id)->fecha_fin_prorroga))}}
                            @else
                            Aún no registra prórroga
                            @endif
                            </td>
                            <!--Prorrogas-->
                        </tr>
